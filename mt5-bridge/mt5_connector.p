...
def candles():
    ...
    if from_ts and to_ts:
        try:
            ...
            start_epoch = int(float(from_ts))
            end_epoch = int(float(to_ts))
            if end_epoch <= start_epoch:
                return jsonify({"status": "error", "message": "invalid candle range", "candles": []})
            date_from = _dt.datetime.fromtimestamp(start_epoch, _dt.timezone.utc)
            date_to = _dt.datetime.fromtimestamp(end_epoch, _dt.timezone.utc)
            rates = mt5.copy_rates_range(used, tf_value, date_from, date_to)
        except Exception as exc:
            return jsonify({"status": "error", "message": f"range request failed: {exc}", "candles": []})
    else:
    ...
    return jsonify({"status": "ok", "symbol": used, "timeframe": timeframe, "count": len(out), "candles": out})
