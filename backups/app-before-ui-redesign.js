...
window.GoldAI = {
  ...
  setTradeMode(mode) {
    const m = String(mode || "manual").toLowerCase() === "auto" ? "auto" : "manual";
    window.GoldAI_Config.TRADE_MODE = m;

    const isAuto = (m === "auto");
    window.GoldAI_Config.AUTO_SIGNAL_ENABLED = isAuto;
    window.GoldAI_Config.AUTO_TRADING_ENABLED = isAuto;

    // Sync AutoSignal engine
    if (window.GoldAI_AutoSignal) {
      if (isAuto) window.GoldAI_AutoSignal.start();
      else window.GoldAI_AutoSignal.stop();
    }
  ...
  loadSettings() {
    try {
      ...
      if (stored) {
        ...
        const initialMode = p.tradeMode || "manual";
        window.GoldAI_Config.TRADE_MODE = initialMode;
        ...
        // Initialize UI selector and sync engine state
        const modeEl = el('tradeModeSelect');
        if (modeEl) modeEl.value = initialMode;
        this.setTradeMode(initialMode);
      } else {
        // Fallback default
        this.setTradeMode("manual");
      }
  ...
  updateRiskUI() {
    ...
    const recommendedLot = this.lastResult && Number.isFinite(Number(this.lastResult.lot))
      ? Number(this.lastResult.lot)
      : 0.01;
    const lot = Math.max(0.01, recommendedLot);
    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set('capitalText', cap);
    set('riskText', risk + "%");
