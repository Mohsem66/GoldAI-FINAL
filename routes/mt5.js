...
router.post('/send-signal', async (req, res) => {
  try {
    const { signal, entry, sl, tp1, tp2, tp3, volume, symbol, mode, clientOrderId, deviation } = req.body;
    ...
    const response = await axios.post(`${PYTHON_SERVER}/execute-trade`, {
      ...
      mode: mode || "manual", clientOrderId, deviation,
...
router.get('/candles', async (req, res) => {
  try {
    ...
    // Optional real date-range mode (NEW, additive) — only forwarded when both
    // are present; the Python bridge falls back to the existing count-based
    // request when they're absent, so this is fully backward compatible.
    const params = { symbol, timeframe, count };
    const { start, end } = req.query;
    if (start && end) { params.start = start; params.end = end; }

    const response = await axios.get(`${PYTHON_SERVER}/candles`, {
      params,
      timeout: (start && end) ? 60000 : 20000 // date-range pulls (e.g. a full year) can take much longer than count-based pulls
    }).catch(() => ({
